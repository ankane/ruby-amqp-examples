...
EventMachine.run do
  AMQP.start do |connection|
    ...
    exchange = channel.topic("cities", :auto_delete => true)
    ...
    queue = channel.queue("asia.hk")
    queue.bind(exchange, :routing_key => "americas.#")
    queue.unbind(exchange, :routing_key => "americas.#")
    queue.bind(exchange, :routing_key => "asia.southeast.hk.#").subscribe do |headers, payload|
